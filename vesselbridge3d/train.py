...
def build_train_parser() -> argparse.ArgumentParser:
    ...
    # train/val lists
    ap.add_argument(
        "--train_list", type=str, default=None, help="Path to training JSON list"
    )
    ap.add_argument(
        "--val_list",
        type=str,
        default=None,
        help="Path to validation JSON list (defaults to train_list)",
    )
    ...
    ap.add_argument(
        ...
        type=str,
        default=None,
        help="[Deprecated] Single JSON used for both train/val if train_list not provided",
    )
    ...
    ap.add_argument(
        ...
        help="Input size e.g., 224 or 512. Must be multiples of 16",
    )

    # augmentations
    ap.add_argument(
        "--aug", action="store_true", help="Enable data augmentation on train set"
    )
    ...
    ap.add_argument(
        "--aug_translate", type=float, default=0.10, help="fraction of size (0-1)"
    )
    ...
    ap.add_argument(
        "--aug_bc_p", type=float, default=0.3, help="brightness/contrast prob"
    )
    ...
    ap.add_argument(
        "--use_3d",
        action="store_true",
        help="Use full 3D model (DINO 2D per-slice -> 3D aggregator -> 3D decoder)",
    )
    ap.add_argument(
        "--depth_min_fg_frac",
        type=float,
        default=0.0005,
        help="Per-depth foreground ratio threshold for 3D. Depth below this are masked out from CE/Dice. 0 to disable",
    )
    ...
    ap.add_argument(
        ...
        type=int,
        default=CROP_D,
        help="Crop size in depth dimension (Z). Use 32 or 48 for high-res.",
    )
    ...
    ap.add_argument(
        "--log_dir",
        type=str,
        default="/path/to/log_dir",
        help="dir path to save logs per epoch",
    )
...
def main(argv=None) -> None:
    ...
    train(
        ...
        aug_vflip_p=args.aug_vflip_p,
        aug_noise_p=args.aug_noise_p,
        aug_noise_std=args.aug_noise_std,
        aug_bc_p=args.aug_bc_p,
