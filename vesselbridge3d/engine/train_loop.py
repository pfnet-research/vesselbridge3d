    ...
    if logits.ndim == 5:  # [B, K, D, H, W]
        if logits.shape[-3:] != y.shape[-3:]:
            logits = F.interpolate(
                logits, size=y.shape[-3:], mode="trilinear", align_corners=False
            )
    else:  # [B, K, H, W]
...
def train(
    ...
    with open(log_file, "a", encoding="utf-8") as f:
        ...
        f.write(
            f"# num_classes={num_classes} epochs={epochs} batch_size={batch_size} "
    ...
    assert not (use_25d and use_3d), (
        "use_25d and use_3d cannot be used at the same time!"
    )

    # Datasets / Loaders
    ds_train = VolumeDataset3D(
        train_list,
        out_size=img_size,
    ...
    ds_val = VolumeDataset3D(
        val_list if val_list is not None else train_list,
        out_size=img_size,
        drop_empty=drop_empty,
        min_fg_frac=min_fg_frac,
        augment=False,
    ...
    logger.info(
        f"[Adapter (slice/frozen)] total={fmt(adapter_slice_total)} | trainable={fmt(adapter_slice_trainable)}"
    )
    logger.info(
        f"[Adapter3D (ConvNeXt)]  total={fmt(adapter3d_total)} | trainable={fmt(adapter3d_trainable)}"
    )
    logger.info(
        f"[Adapter Proj/Gates]    total={fmt(proj_gate_total)} | trainable={fmt(proj_gate_trainable)}"
    )
    logger.info(
        f"[Aggregator (wrapper)]  total={fmt(aggregator_total)} | trainable={fmt(aggregator_trainable)}"
    )
    logger.info(
        f"[FFA core]              total={fmt(ffa_core_total)} | trainable={fmt(ffa_core_trainable)}"
    )
    logger.info(
        f"[Segmentation heads]    total={fmt(seg_total)} | trainable={fmt(seg_trainable)}"
    )
    logger.info(
        f"[ViT encoder (frozen)]  total={fmt(vit_total)} | trainable={fmt(vit_trainable)}"
    )
    logger.info("--------------------------------------------")
    logger.info(
        f"[MODEL TOTAL]           total={fmt(model_total_all)} | trainable={fmt(model_total_trainable)}"
    )
    logger.info("============================================")
    ...
    class_weights = make_class_weights(
        num_classes, counts, bg_weight=bg_weight, use_mfb=use_mfb
    )
    if init_prior_bias:
    ...
    criterion = DC_and_CE_loss(
        ...
        use_skelrecall=use_skelrecall,
        skelrecall_weight=skelrecall_weight,
        skelrecall_iters=skelrecall_iters,
    )
    ...
    if os.path.isfile(latest_ckpt_path):
        ...
        logger.info(
            f"[INFO] Resumed from Epoch {checkpoint['epoch']}. Next Epoch: {start_epoch}"
        )
    else:
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        ev_ce, ev_dice, ev_topo, ev_total = eval_epoch(
            model, loader_eval, criterion, device
        )
        ...
        if epoch % 20 == 0:
            ...
            gt_vis = colorize(gt, palette)
            pr_vis = colorize(pr, palette)
            concat = np.concatenate([gt_vis, pr_vis], axis=1)
            out_path = os.path.join(
                "/path/to/vis_outputs/", f"epoch{epoch:03d}_sample0.png"
            )
        ...
        if ev_dice < best_dice:
            ...
            logger.info(
                f"[INFO] Saved checkpoint: minibaseline_best_dice.pt (loss={best_loss:.4f}), dice={best_dice:.4f}"
            )
        ...
        if ev_total < best_loss:
            ...
            logger.info(
                f"[INFO] Saved checkpoint: minibaseline_best_total.pt (loss={best_loss:.4f}), dice={best_dice:.4f}"
            )
