...
class SegModel3D_UNETRLite(nn.Module):
    ...
    def __init__(
        ...
        self.adapter_slice = FrozenAdapter()
        [p.requires_grad_(False) for p in self.adapter_slice.parameters()]
        self.encoder = FrozenDINOv3ViTS16(default_layers=vit_layers)
        ...
        self.f2_proj_ref = nn.Conv3d(
            48, 64, kernel_size=1, bias=False
        )  # F2 -> 64 (refine用)
        self.f3_proj_C = nn.Conv3d(
            64, C, kernel_size=1, bias=False
        )  # F3 -> C (ViT hidden)
