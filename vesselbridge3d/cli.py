...
Exposed both as ``python -m vesselbridge3d.train`` / ``...inference`` (via the
thin shim modules) and as the ``vesselbridge3d-train`` / ``vesselbridge3d-infer``
...
from .engine import run_inference, train
...
def inference_main(argv=None) -> None:
